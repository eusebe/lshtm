...
output: 
  lshtm::lshtm:
    ...
    includes:
      in_header: lshtm/header.tex
bibliography: lshtm/demo.bib
biblio-style: abbrvnat
beameroption: "show notes"
toc: true
vignette: >
  %\VignetteIndexEntry{lshtm LSHTM Demo}
  %\VignetteKeywords{lshtm,vignette}
  %\VignettePackage{lshtm}
...
\scriptsize This vignette actually uses **lshtm** theme, which needs the \emph{Carlito} font (just Google it).
...
\scriptsize Small caps not working with the font used in the **lshtm** theme.
...
\bigskip
\scriptsize This uses \LaTeX\ for animation. The next slides uses RMarkdown
...
\scriptsize The right side uses the `\metroset{block=fill}` option, 
but this is a metropolis option: not really working with **lshtm** theme. 
Blocks can also used in Markdown using `###` (if slide-level=2).
...
## Colortheme (with **lshtm** theme)

The **lshtm** theme has three colortheme:

- `lshtmgreen` (default)
- `lshtmlightgreen`
- `lshtmreverse`

Simply set the `colortheme: lshtmreverse` option in the YAML header of your 
markdown document.

## Aspect ratio (with **lshtm** theme)

Like other themes, the aspect ratio can be set to 16/9 by setting     
`classoption: aspectratio=169` in the YAML header. 

But to get a correct title page, you also need to set      
`titlepage: titlepage169.png`.
